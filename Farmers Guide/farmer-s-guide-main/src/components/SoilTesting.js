...
function SoilTesting() {
  ...
  // Fetch soil types from the backend
  useEffect(() => {
    axios
      .get("http://localhost:5002/api/soil-types")
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5002/api/crops")
      .then((response) => {
  ...
  // Fetch fertilizers based on selected soil type & crop type
  useEffect(() => {
    if (selectedSoilType && selectedCropType) {
      axios
        .get("http://localhost:5002/api/fertilizers", {
          params: { soilType: selectedSoilType, cropType: selectedCropType },
        })
        .then((response) => {
          const fertilizerData = response.data[0]; // Take the first matching fertilizer
          if (fertilizerData) {
            setRecommendations({
              ...
              series: [
                {
                  name: "Nutrients",
                  data: [
                    fertilizerData.nitrogen || 0,
                    fertilizerData.phosphorus || 0,
                    fertilizerData.potassium || 0,
                  ],
                },
              ],
              options: {
                ...chartData.options,
                xaxis: { categories: ["Nitrogen", "Phosphorus", "Potassium"] },
              },
            });
          } else {
            setRecommendations({});
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-lg">
        <p className="text-lg mb-6">
          Explore detailed soil testing procedures and get personalized
          recommendations based on your soil and crop type.
        </p>
        ...
        <div className="mb-6 flex space-x-4">
          <div className="flex-1">
            ...
            >
              ...
              {soilTypes.map((soil) => (
                <option key={soil._id} value={soil.name}>
                  {soil.name}
                </option>
              ))}
            </select>
        ...
        {Object.keys(recommendations).length > 0 && (
          <div className="mb-8">
            ...
            <ul className="list-disc ml-6 mb-4">
              <li>
                <strong>Nitrogen:</strong> {recommendations.nitrogen} g/kg
              </li>
              <li>
                <strong>Phosphorus:</strong> {recommendations.phosphorus} g/kg
              </li>
              <li>
                <strong>Potassium:</strong> {recommendations.potassium} g/kg
              </li>
            </ul>
          </div>
        )}
